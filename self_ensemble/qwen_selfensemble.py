...
def parse_qwen_prompt(prompt: str, tokenizer):
    inputs = tokenizer(prompt, padding=False, return_tensors="pt")
    full_tokens = inputs.input_ids
    assistant_marker = "<|reserved_special_token_0|><|im_end|>\n<|im_start|>assistant\n<|im_end|>"
...
class LLM_Agent:
    ...
    def output_sample_logits(self, input_text):
        ...
        q_end, answer_blocks, ending_start_index = parse_qwen_prompt(input_text, self.tokenizer)
...
def main():
    ...
    for ques_idx in tqdm(range(dataset_size)):
        ...
        prompt = [{"role": "user", "content": sampled_prompts}] + \
                     [{"role": "assistant", "content": ""}]
        prompt_in_template = assistance.tokenizer.apply_chat_template(prompt, tokenize=False)
        logits = assistance.output_sample_logits(prompt_in_template)
    ...
    results_fname = f"./json/model_{model_name}_dataset_{dataset_name_new}_sample4_20trials_seed0.json"
