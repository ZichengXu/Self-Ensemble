...
def generate_custom_positional_ids(question_end_index, answer_slices, ending_index, total_tokens):
    ...
    pos_ids_tensor = torch.tensor(pos_ids, dtype=torch.long).unsqueeze(0)
...
def parse_mistral_prompt(prompt: str, tokenizer):
    inputs = tokenizer(prompt, padding=False, return_tensors="pt")
    full_tokens = inputs.input_ids
    assistant_marker = "<|reserved_special_token_0|> [/INST]"
    ...
    for line in answer_block_lines:
        ...
        answer_blocks.append([start_token_index, end_token_index+1])
...
class LLM_Agent:
    ...
    def output_sample_logits(self, input_text):
        ...
        q_end, answer_blocks, ending_start_index = parse_mistral_prompt(input_text, self.tokenizer)
